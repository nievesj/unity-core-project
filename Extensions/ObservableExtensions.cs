...
namespace Core.Common.Extensions.Observable
{
    public static class ObservableExtensions
    {
        ...
        public static IObservable<UnityEngine.Object> ToObservable(this AssetBundleRequest asyncOperation)
        {
            ...
            return UniRx.Observable.FromCoroutine<UnityEngine.Object>((observer, cancellationToken) => RunAssetBundleRequestOperation(asyncOperation, observer, cancellationToken));
        }
        ...
        {
            return UniRx.Observable.Create<Unit>(observer =>
                {
                    task.Run(_ =>
                    {
        ...
        {
            particle.Play();
            return UniRx.Observable.FromCoroutine<ParticleSystem>(observer => OnParticleStopAsync(observer, particle));
        }
        ...
        {
            particle.Play();
            return UniRx.Observable.FromCoroutine<ParticleSystem>(observer => OnParticleStopAsync(observer, particle, seconds));
        }

        public static IObservable<ParticleSystem> StopRx(this ParticleSystem particle, float burnOutTime = 1.5f)
        {
            return UniRx.Observable.FromCoroutine<ParticleSystem>(observer => OnParticleStopAsync(observer, particle, 0, burnOutTime));
        }
